...
const create = async function(req, res){
    ...
    const body = req.body;
    if(!body.unique_key && !body.email && !body.phone && !body.amount){
        return ReE(res, 'Please enter an email or phone number to register.');
    } else if(!body.password){
        return ReE(res, 'Please enter a password to register.');
    } else if (!body.amount) {
        return ReE(res, 'Please enter the amount you are interested in investing (we just need a rough estimate)')
    }
    else{
        let err, user;
